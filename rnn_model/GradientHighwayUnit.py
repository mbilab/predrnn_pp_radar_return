...
class gradient_highway(nn.Module):    
    def __init__(self, layer_name, filter_size, num_features, batch, height, width, device, tln=False):
        ...
        self.z_norm = nn.LayerNorm([self.num_features*2, self.height, self.width])
        self.x_norm = nn.LayerNorm([self.num_features*2, self.height, self.width])
...
#         self.x_norm = nn.BatchNorm2d(self.num_features*2)
    ...
    def init_state(self):
        ...
        return torch.zeros((self.batch,self.num_features,self.height,self.width), dtype=torch.float32).to(self.device)
